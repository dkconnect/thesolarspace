...
const argv = require("minimist")(process.argv.slice(2), {
  alias: { s: "size", o: "overlap", r: "rotate" },
  default: { size: 256, overlap: 0, rotate: true },
});
...
(async () => {
  if (fs.existsSync("raw")) {
    ...
    if (!fs.existsSync("computed")) fs.mkdirSync("computed");
    ...
    for (const file of files) {
      ...
      for (let x = 0; x + size <= width; x = Math.floor(x + size * (1 - overlap))) {
        for (let y = 0; y + size <= height; y = Math.floor(y + size * (1 - overlap))) {
          ...
          await image.extract({ left: x, top: y, width: size, height: size })
            .jpeg({ quality: 100, chromaSubsampling: "4:4:4" })
            .toFile(output);
          // Rotations
          for (let r = 1; r < (rotate ? 4 : 1); r++) {
            await sharp(output).rotate(r * 90)
              .toFile(output.replace(/-[0-9a-f]+\.(.+?$)/, (m, a) => `-${uuid()}.${a}`));
          }
        }
      }
      // Update bar
      bar.increment();
      if (bar.value === files.length) bar.stop();
    }
  } else {
